...
class VideoGenerator:
    def __init__(self, width=1080, height=1920, max_pieces_video_in_one_video: int = 10, min_delay: float = 1, max_delay: float = 5) -> None:
        ...
        video_list, audio_list = (list(settings.VIDEOS.glob("*.mp4")),
                                  list(settings.AUDIOS.glob("*.mp3")))
        fonts_list, texts_list = (list(settings.FONTS.glob("*.ttf")),
                                  list(settings.TEXTS.glob("*.txt")))
    ...
    def run(self, width=1080, height=1920, max_pieces_video_in_one_video: int = 10, min_delay: float = 1, max_delay: float = 5, count: int = 1):
        self.__init__(width, height, max_pieces_video_in_one_video, min_delay, max_delay)
    ...
    def output_video(self, video_stream):
        ...
        # Выгрузка видео
        ffmpeg.output(video_stream, audio_stream, str(settings.RESULTS / f"{time.time():.0f}.mp4"), **{"qscale:v": "3", "vsync": "2"}, shortest=None).run()
